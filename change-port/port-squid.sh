...
sqd3="$(cat ~/log-install.txt | grep -w "Squid Proxy" | awk '{print $7}' | cut -d: -f2|sed 's/ //g')"
echo -e "\e[0;31m.-----------------------------------------.\e[0m"
...
cek=$(netstat -nutlp | grep -w $squid)
if [[ -z $cek ]]; then
sed -i "s/$sqd3/$squid/g" /etc/squid/squid.conf
sed -i "s/$sqd3/$squid/g" /root/log-install.txt
/etc/init.d/squid restart > /dev/null
echo -e "\e[032;1mPort $squid modified successfully\e[0m"
else
echo -e "\e[1;31mPort $squid is used\e[0m"
